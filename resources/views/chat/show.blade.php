...
<head>
    ...
    <title>Chat avec {{ $user->name }} – Maison Dagaz</title>
...
<nav class="navbar">
    ...
    <a href="{{ route('chat.index') }}" class="navbar-back">← Tous les messages</a>
</nav>
    ...
    <aside class="sidebar">
        ...
        @foreach ($contacts as $contact)
            <a href="{{ route('chat.show', $contact->id) }}"
               class="contact-item {{ $contact->id === $user->id ? 'active' : '' }}">
                <div class="avatar sm">{{ strtoupper(substr($contact->name, 0, 1)) }}</div>
                <div class="contact-info">
                    <div class="contact-name">{{ $contact->name }}</div>
                    <div class="contact-preview">{{ Str::limit($contact->last_message ?? '—', 28) }}</div>
                </div>
    ...
    <div class="chat-window">
        ...
        <div class="messages-area" id="messages-area">
            @foreach ($messages as $msg)
                ...
                <div class="msg-row {{ $mine ? 'mine' : '' }}" data-id="{{ $msg->id }}">
                    @if (!$mine)
...
<script>
    ...
    function appendMessage(msg) {
        ...
        const avatarLetter = '{{ strtoupper(substr($user->name,0,1)) }}';
